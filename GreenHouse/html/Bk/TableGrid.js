...
Object systemSet	函數相關設定。id：表格ID、set：表格設定、width：表格寬度
*/
function EditTableGrid(data,systemSet)
{	// 資料處理轉換：時間轉換、格式轉換
	var newData = new Array();
	var dataLength = data[0].length;
	//
		...
		{	// data值
			...
			{	
				case "datetime":
					...
					break;
				case "select":
					//dataTemp[key1] = data_code[key1][data_value];
	...
	$("#update").click(function()
	{
		...
		{	// 編輯表格設定
			...
			//
			myGrid.jqGrid('editGridRow',gr,updateSet);
		}
...
String navID			導航列ID
*/
function EditTableGridSet(titleName,titleWidth,rowType,rowOptions,tableHeight,navID)
{
	var options = new Object();
	options.data = new Array();
	options.datatype = 'local';
