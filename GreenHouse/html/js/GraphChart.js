...
Object systemSet	函數相關設定。id：圖表ID、set：圖表設定
*/
function LineHistoryChart(data,systemSet)
{	
	var myChart = echarts.init(document.getElementById(systemSet.id),"macarons");
	var object = systemSet.set;
	// 數據放入折線圖設定
	for(a=0;a<data.length;a++)
	{
		...
		for(b=0;b<data[a].length;b++)
		{
			...
			switch (device)
			{
				...
				case "Area2_Spritz":
				case "Area2_FCU":
					if ( data[a][b].value == "true" )
						temp.push("on");
					else if ( data[a][b].value == "false" )
						temp.push("off");
					else
						temp.push(data[a][b].value);
					break;
...
Array dataGridSet		資料表格位置
Array deviceNameSet		設備名稱
Object set				相關設定
*/
function LineHistoryChartSet(dataNameSet,dataGridSet,deviceNameSet,set)
{
	...
	{	// 資料設置
		switch (dataGridSet[i])
		{
			...
			default:
				...
				options.series[i].name = dataNameSet[i];
				options.series[i].type = "line";
				options.series[i].step = "end";
				options.series[i].data = new Array();
				options.series[i].smooth = false;
				//options.series[i].smoothMonotone = 'x';
				var gridLocation = parseInt(dataGridSet[i]);
				// y軸坐標多一個
				options.series[i].xAxisIndex = gridLocation;
				options.series[i].yAxisIndex = gridLocation+1;
		}
	}
   	// 折線圖標題
   	...
   	// 表格區域grid
	options.grid = new Array();
   	options.grid[0] = new Object();
   	options.grid[0].left = 50;
   	options.grid[0].right = 50;
   	options.grid[0].height = '36%';
   	
   	for (a=0 ; a<set.statusDevice ; a++)
   	{
   		var temp = new Object();
	   	temp.left = 50; 
	   	temp.right = 50; 
	    temp.top = 56+a*5.5 +"%";
	   	temp.height = '5.5%';
	   	options.grid.push(temp)
   	}
   	// x軸
   	options.xAxis = new Array();
   	options.xAxis[0] = new Object();
   	options.xAxis[0].name = set.xName;
   	options.xAxis[0].type = 'time';
   	options.xAxis[0].scale = true;
	// 最大與最小
   	options.xAxis[0].min = set.xMin;
   	options.xAxis[0].max = set.xMax;
   	options.xAxis[0].axisLabel = new Object();
   	options.xAxis[0].axisLabel.formatter = function (value, index) 
   	{	// 格式化成月/日，只在第一个刻度或是增加值顯示年份
	    var date = new Date(value);
	    var val = date.customFormat("#YYYY#/"+"#MM#/"+"#DD#"+"\n#hhhh#:"+"#mm#:"+"#ss#");
	  //  if (index === 0) 
	...
	for (a=0 ; a<set.statusDevice ; a++)
   	{
		...
		temp.gridIndex = a+1;
	   	temp.type = 'time';
	   	temp.scale = true;
	    temp.name = deviceNameSet[a];
	    temp.offset = 0;
	    temp.nameGap = 1;
		temp.axisLabel = new Object();
		temp.axisLabel.show = false;
		// 最大與最小
		temp.min = set.xMin;
		temp.max = set.xMax;
	   	options.xAxis.push(temp)
   	}
	// y軸
	// 溫度
	options.yAxis = new Array();
   	options.yAxis[0] = new Object();
	options.yAxis[0].name = set.yName1 + " " + set.yUnit1;
	options.yAxis[0].scale = false;
	options.yAxis[0].min = 10;
	...
	options.yAxis[1].name = set.yName2 + " " + set.yUnit2;
	options.yAxis[1].scale = false;
	options.yAxis[1].min = 40;
	...
	for (a=0 ; a<set.statusDevice ; a++)
   	{
	    ...
	    temp.data= ['off','on'];
	    temp.axisLabel = new Object();
	    temp.axisLabel.interval = 0;
	   	options.yAxis.push(temp)
   	}
	//
    options.axisPointer = new Object();
    options.axisPointer.link = new Object();
    options.axisPointer.link.xAxisIndex = 'all';
	// 縮放設定 dataZoom
	options.dataZoom = new Object();
	options.dataZoom.show = false;
	options.dataZoom.realtime = true;
	options.dataZoom.start = 0;
	options.dataZoom.end = 100;
	// 縮放可控制x軸數量
	var tempArray = new Array();
	for (a=0 ; a<set.statusDevice+1 ; a++)
	   	tempArray.push(a);
	options.dataZoom.xAxisIndex = tempArray;
	// 提示框
	options.tooltip = new Object();
	options.tooltip.trigger = 'axis';
	options.tooltip.axisPointer = new Object();
	options.tooltip.axisPointer.axis = "x";
...
Object systemSet	函數相關設定。id：圖表ID、set：圖表設定
*/
function SunBarChart(data,systemSet)
{	
	var myChart = echarts.init(document.getElementById(systemSet.id),"macarons");
	var object = systemSet.set;
	// X軸坐標-全部集合
...
Object myChart		圖表元件
*/
function getSlotValue(myChart)
{
	// 日照時數設定
	var system = new Object();
	system.dataname = "out";
	system.htmlid = 'sun';
	system.chart = myChart;
	// Slot 資料
	NiagaraSlotData("station:|slot:/GH/Sun_hr_input","SlotData2Chart",system);
}
// 資料放入圖表
function SlotData2Chart(data,systemSet)
{	
	document.getElementById(systemSet.htmlid).innerHTML= data;
	var object = new Object();
	object.series = new Array();
	object.series[0] = new Object();
	object.series[0].markLine = new Object();
	object.series[0].markLine.data = new Array();
	object.series[0].markLine.data[0] = new Object();
	object.series[0].markLine.data[0].yAxis = data;
	var myChart = systemSet.chart;
	myChart.setOption(object);
}
...
String yunit			y軸單位
*/
function BarChart1Set(datanameset,title,xname,yname1,yunit1)
{
	var options = new Object();
	// 資料
	options.series = new Array();
	...
	// y軸
	options.yAxis = new Object();
	options.yAxis.name = yname1 + " " + yunit1;
	options.yAxis.scale = true;
	options.yAxis.min = 0;
	options.yAxis.axisLabel = new Object();
	options.yAxis.axisLabel.formatter = '{value}';
...
//
function LineChartSet22(datanameset,title,xname,yname,yunit,alarm)
{
	var options = new Object();
	// 資料
	options.series = new Array();
	for(i=0 ; i<datanameset.length ; i++)
	{
   	...
   	{	// 格式化成月/日，只在第一个刻度或是增加值顯示年份
	    var date = new Date(value);
	    var val = date.customFormat("#hhhh#:"+"#mm#");
	  //  if (index === 0) 
	  //      val = date.customFormat("#YYYY#") + "\n" + val;
	    return val;
	};
	options.xAxis.axisLabel.textStyle = new Object();
	options.xAxis.axisLabel.textStyle.color = "#1e6e8c";
...
String yunit			y軸單位
*/
function BarChartSet22(datanameset,title,xname,yname,yunit,chartid)
{
	var options = new Object();
	// 資料
	options.series = new Array();
	for(i=0 ; i<datanameset.length ; i++)
	{
		...
		options.series[i].name = datanameset[i];
		options.series[i].type = "bar";
		options.series[i].barMaxWidth = 80;
		options.series[i].data = new Array();
		//標準線設置
		options.series[i].markLine = new Object();
		options.series[i].markLine.itemStyle = new Object();
		options.series[i].markLine.itemStyle.normal = new Object;
		options.series[i].markLine.itemStyle.normal.lineStyle = new Object();
		options.series[i].markLine.itemStyle.normal.lineStyle.type = "solid";
		options.series[i].markLine.itemStyle.normal.lineStyle.shadowBlur = 2;
		options.series[i].markLine.itemStyle.normal.lineStyle.color = "#000000";
		options.series[i].markLine.itemStyle.normal.lineStyle.opacity = 0.4;
		options.series[i].markLine.data = new Array();
		options.series[i].markLine.data[0] = new Object();
		options.series[i].markLine.data[0].name = "標準線";
		options.series[i].markLine.data[0].yAxis = 0;
	}
   	// 柱狀圖標題
   	options.title = new Object();
   	options.title.textStyle = new Object();
	...
	options.xAxis.axisLabel = new Object();
   	...
   	{	// 格式化成月/日，只在第一个刻度或是增加值顯示年份
	    var date = new Date(value);
	    var val = date.customFormat("#MM#/"+"#DD#");
	  //  if (index === 0) 
	  //      val = date.customFormat("#YYYY#") + "\n" + val;
	    return val;
	};
	    ...
	    {	// BajaScript
		    ...
		    {	// 執行
		    	ok: function ()
		    	{
		    		...
		    		object.series[0].markLine.data = new Array();
		    		object.series[0].markLine.data[0] = new Object();
		    		object.series[0].markLine.data[0].yAxis = inputText;
		    		// 讀取圖表元件
...
//暫時展示 之後會刪
function LineChartSet33(datanameset,title,xname,yname,yunit,alarm,dateAry)
{
	var options = new Object();
	// 資料
	options.series = new Array();
	...
	options.xAxis.axisLabel.textStyle = new Object();
	options.xAxis.axisLabel.textStyle.color = "#1e6e8c";
	options.xAxis.splitNumber = 7;
	options.xAxis.axisLabel.padding = [0,0,0,0];
...
暫時展示，之後刪掉
*/
function BarChart22(data,systemSet)
{	
	data[0][0]["date"] = GetDateStr(-6)+" 00:00:00.0";
	data[0][1]["date"] = GetDateStr(-5)+" 00:00:00.0";
	data[0][2]["date"] = GetDateStr(-4)+" 00:00:00.0";
	data[0][3]["date"] = GetDateStr(-3)+" 00:00:00.0";
	data[0][4]["date"] = GetDateStr(-2)+" 00:00:00.0";
	data[0][5]["date"] = GetDateStr(-1)+" 00:00:00.0";
	data[0][6]["date"] = GetDateStr(0)+" 00:00:00.0";
	data[0][6]["value"] = 6.1;
	var myChart = echarts.init(document.getElementById(systemSet.id),"macarons");
	var object = systemSet.set;
	// X軸坐標-全部集合
...
暫時展示，之後刪掉
*/
function LineChart33(data,systemSet)
{	
	...
	for(a=0;a<data.length;a++)
	{
		...
		for(b=0;b<data[a].length;b++)
		{
			data[a][b]["date"] = GetDateStr(0)+" "+b+":00:00.0";
			var temp = new Array();
			temp.push(dateFoematISO(data[a][b].date));
			temp.push(data[a][b].value);
			dataset.push(temp);
...
暫時展示，之後刪掉
*/
function LineChart44(data,systemSet)
{		
	//alert(JSON.stringify(data));
	...
	for(a=0;a<data.length;a++)
	{
		...
		{
			var temp = new Array();
			data[a][b].date=GetDateStr(b-7)+" 00:00:00.0";
			temp.push(dateFoematISO(data[a][b].date));
			temp.push(data[a][b].value);
			dataset.push(temp);
		}
